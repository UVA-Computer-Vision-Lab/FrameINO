'''
    This is for the WAN2.2 5B (TI2V) version VDM training with Motion Control.
'''
...
from pipelines.pipeline_wan_i2v_motion import WanImageToVideoPipeline
...
def get_args():
    ...
    parser.add_argument(
                            "--config_path",
                            type = str,
                            default = "config/train_wan_motion.yaml",
                            # required = True,
...
def log_validation(
    ...
    store_folder_path = os.path.join(validation_store_path, "Process"+str(accelerator.process_index))
    os.makedirs(store_folder_path, exist_ok=True)  # Should not have the path exists (unless there is a conflict)
    ...
    for idx, batch in enumerate(val_dataloader):
        ...
        # print("Node Rank is ", int(os.environ.get("NODE_RANK", 0))  )
...
def video_tensor_to_vae_latent(video_tensor, vae, device):
    ...
    # First frame Extract and Add Padding
    first_frame = video_tensor[:, :, :1].clone()        # Must use [:1] for keeping that dimension purposes
    # first_frame_padded = torch.cat(
    #                                 [first_frame, first_frame.new_zeros(first_frame.shape[0], first_frame.shape[1], video_tensor.shape[2] - 1, video_tensor.shape[3], video_tensor.shape[4])], dim=2
    #                             )
...
def main(config, args):
    ...
    ##################################### Add new learnable variables or modify the setting #############################################

    # Applying ConvIn Merge to reuse most of the weights

    # Append a new proj for the Patch_embed
    ...
    new_in_channels = transformer.config["in_channels"] + 48        # Extra channel increase for the motion cond

    # Clone the original weight
    _weight = transformer.patch_embedding.weight.clone().detach()   # [3072, 32, 2, 2]
    _bias = transformer.patch_embedding.bias.clone().detach()       # [3072]

    # Replace Part of channels (32 / 48) to be the original weight
    transformer.patch_embedding = zero_module(nn.Conv3d(new_in_channels, inner_dim, kernel_size=patch_size, stride=patch_size))
    transformer.patch_embedding.weight.data[:,:original_in_channels,:,:] = _weight.data
    transformer.patch_embedding.bias.data = _bias.data
    ...
    transformer.to(accelerator.device).to(weight_dtype)         
    ...
    def collate_fn(examples):
        ...
        # First frame np
        first_frame_np = [example["first_frame_np"] for example in examples]   
    ...
    train_datasets = [
                        VideoDataset_Motion(config, download_folder_path, train_csv_relative_path, config["train_video_relative_path"])
    ...
    mixed_sampler = MixedBatchSampler(
                                        ...
                                        drop_last = True,
                                        shuffle = True,
                                        # Don't Use generator: it will be fixed dataset order in resume
                                    )
    ...
    val_dataset = VideoDataset_Motion(config, download_folder_path, validation_csv_relative_path, config["validation_video_relative_path"])
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                first_frame_np = batch["first_frame_np"]
                

                # Encode Video and Traj tensor to latnets
                with torch.no_grad():
                    video_latents, first_frame_latent = video_tensor_to_vae_latent(video_tensor, vae, accelerator.device)
                ...
                timesteps = noise_scheduler.timesteps[indices].to(device=video_latents.device)      # NOTE: Don't cast to other weight dtype; else, cannot align sigmas dtype
                ...
                noisy_model_input = torch.cat([noisy_video_latents, traj_latents], dim=1)     # The channel dim 48 + 48 = 96
