...
from pipelines.pipeline_cogvideox_i2v_motion import CogVideoXImageToVideoPipeline, get_resize_crop_region_for_grid
...
def get_args():
    ...
    parser.add_argument(
                            "--config_path",
                            type=str,
                            default="config/train_cogvideox_motion.yaml",
...
def video_tensor_to_vae_latent(video_tensor, vae, device):
    ...
    # Preprocess
    video_tensor = video_tensor.to(device, dtype=vae.dtype)     
    video_tensor = video_tensor.permute(0, 2, 1, 3, 4)  # [B, C, F, H, W]
    first_frame = video_tensor[:, :, :1].clone()
...
def main(config, args):
    ...
    _weight = transformer.patch_embed.proj.weight.clone().detach()   # [3072, 32, 2, 2]
    _bias = transformer.patch_embed.proj.bias.clone().detach()       # [3072]
    
    # Half the activation magnitude to weaken the previous influece
    # _weight *= 0.5

    # Replace Part of channels (32 / 48) to be the original weight
    transformer.patch_embed.proj = zero_module(nn.Conv2d(48, embed_dim, kernel_size=(patch_size, patch_size), stride=patch_size))
    transformer.patch_embed.proj.weight.data[:,:32,:,:] = _weight.data
    transformer.patch_embed.proj.bias.data = _bias.data
    

    # Update config
    transformer.config["in_channels"] = 48
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                batch_size, latent_num_frames, num_channels, latent_height, latent_width = video_latents.shape       # NOTE: now we set the batch size to be 1, to adapt arbitrary resolution inputs
                ...
                traj_latents = traj_latents.to(dtype = weight_dtype)    # [B, F, C, H, W]
                first_frame_latent = first_frame_latent.to(dtype = weight_dtype)  # [B, F, C, H, W] 
                ...
                # Note: In the motion version, we cancatenate noisy latnets, first frame latnet, and tracjector latents channel-wise together
                noisy_model_input = torch.cat([noisy_video_latents, first_frame_latent, traj_latents], dim=2)     # The channel dim 16 -> 32


                # Prepare rotary embeds
                image_rotary_emb = (
                                        prepare_rotary_positional_embeddings(
                                                                                height = pixel_height,        # latent height width and number of frames
                                                                                width = pixel_width,
