...
if __name__ == "__main__":
    ...
    # Output Dir  
    store_parent_folder_name = "results_FrameIn"       # Output
    assert(os.path.exists(download_folder_path))
    ...
    preset_width = 640   
    preset_num_frames = 49
    num_test = 200                  # Max setting 200 (actually the evaluation dataset is less than this number)



    # Prepare
    test_csv_folder_path = os.path.join(download_folder_path, "dataset_csv_files/evaluation_formal_perfect_openvid_FrameIn")
    assert(os.path.exists(test_csv_folder_path))
    dataset_folder_path = os.path.join(download_folder_path, "video_dataset/evaluation_FrameIn")
    assert(os.path.exists(dataset_folder_path))
    ID_folder_path = os.path.join(download_folder_path, "video_dataset/evaluation_ID_FrameIn")
    assert(os.path.exists(ID_folder_path))


    # Replace information (Especially the height and width information, if needed)
    config = {}
    config['dataset_folder_path'] = dataset_folder_path
    config['ID_folder_path'] = ID_folder_path
    ...
    config["end_skip"] = 0
    config["dot_radius"] = 6
    config["drop_FrameIn_prob"] = 0.0       # For Validtion of Frame In, we must have ID
    config["text_mask_ratio"] = 0.0         # Must have a text prompt
    config["faster_motion_prob"] = 0.0      # No fast running
    config["point_keep_ratio_regular"] = 0.5    # Recommend around 0.5, the training is also around 0.5
    config["point_keep_ratio_ID"] = 0.5         # Recommend around 0.5, the training is also around 0.5
    ...
    test_dataset = VideoDataset_Motion_FrameINO(config, test_csv_folder_path, strict_validation_match = True)
    ...
    for instance_idx, batch in enumerate(test_dataloader):
        ...
        imageio.mimsave(os.path.join(store_folder_path, "gt_padded_video.mp4"), video_gt_np, fps=12)
        ...
        # Write as Frames for Generated Video
        gen_frame_cropped = []
