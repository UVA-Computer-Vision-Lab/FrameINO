...
class CogVideoXPatchEmbed(nn.Module):
    def __init__(
        ...
        # Extra channels for the motion abjustment
        ...
        if use_positional_embeddings or use_learned_positional_embeddings:  
            ...
            # print("mean value of pos_embedding is ", torch.mean(pos_embedding))

            # if use_FrameIn: # Position Embedding for the reference
            #     pos_embedding_full = self._get_positional_embeddings(sample_height, sample_width, sample_frames+self.temporal_compression_ratio)
            #     ID_start_idx = self.max_text_seq_length + (sample_height * sample_width * ((sample_frames - 1) // self.temporal_compression_ratio + 1) // (self.patch_size ** 2))
            #     pos_embedding_ID_reference = pos_embedding_full[:, ID_start_idx:, :]        # 1350 tokens for one frame
            #     self.register_buffer("pos_embedding_ID_reference", pos_embedding_ID_reference, persistent=persistent)
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        if self.patch_size_t is None:
            ...
            image_embeds = self.proj(image_embeds)      # HACK: Project channels to 1920 dim, which is the same dim of Text
        ...
        if self.use_positional_embeddings or self.use_learned_positional_embeddings:
            ...
            # Directly reuse the pos_embedding available      
            if self.use_FrameIn:
                first_frame_token_num = (self.pos_embedding.shape[1] - self.max_text_seq_length) // (num_frames - 1)        # Minus the number of frames
                # NOTE: the following place has bug where it overlap with the text token
                pos_embeds = torch.cat([self.pos_embedding, self.pos_embedding[:, text_seq_length:text_seq_length+first_frame_token_num].clone()], dim=1)     # Append the pos_embeds in the token-wise dimension

            else:
                pos_embeds = self.pos_embedding


            # The training and default resolution is not matched, so we need to do resize
            if (
                ...
                if self.use_FrameIn:
                    post_time_compression_frames = post_time_compression_frames + 1

                emb_size = embeds.size()[-1]
                pos_embeds_without_text = pos_embeds[:, text_seq_length: ].view(1, post_time_compression_frames, post_patch_height, post_patch_width, emb_size)
                pos_embeds_without_text = pos_embeds_without_text.permute([0, 4, 1, 2, 3])

                # Resize the Absolute position embeddings for variable resolution cases
                pos_embeds_without_text = F.interpolate(pos_embeds_without_text, size = [post_time_compression_frames, height // self.patch_size, width // self.patch_size], mode='trilinear', align_corners=False) 
                pos_embeds_without_text = pos_embeds_without_text.permute([0, 2, 3, 4, 1]).view(1, -1, emb_size)
                pos_embeds = torch.cat([pos_embeds[:, :text_seq_length], pos_embeds_without_text], dim = 1) # Concat the text back
                pos_embeds = pos_embeds[:, : text_seq_length + seq_length]


            # Add position embeddings to the original embeddings
            pos_embeds = pos_embeds.to(dtype = embeds.dtype)
...
def apply_rotary_emb(
    ...
    """
    if use_real:    # HACK: This is True usually
        cos, sin = freqs_cis  # [S, D]
        cos = cos[None, None]
        sin = sin[None, None]
        cos, sin = cos.to(x.device), sin.to(x.device)

        if use_real_unbind_dim == -1:       # HACK: Pass this branch
